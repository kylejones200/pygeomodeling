...
class DeepGPExperiment:
    ...
    def train_model(
        ...
        **model_kwargs
        ...
        unique_likelihood_params = [p for p in likelihood_params if id(p) not in all_param_ids]
    ...
    def plot_comparison_results(self):
        ...
        fig.suptitle('Deep GP vs Traditional GP Comparison - SPE9 Reservoir', fontsize=16)
        ...
        ax.plot([pred_data['true'].min(), pred_data['true'].max()], 
                [pred_data['true'].min(), pred_data['true'].max()], 'r--', lw=2)
        ...
        for name in model_names:
            ...
            print(f"  R²: {metrics['r2_score']:.4f} | RMSE: {metrics['rmse']:.4f} | Time: {metrics['training_time']:.1f}s")
