...
class SPE9Toolkit:
    ...
    def visualize_results(
        ...
        *, 
        ...
        # Original PERMX
        im1 = axes[0, 0].imshow(
            self.grid_data.permx_3d[:, :, z_slice].T, 
    ...
    def _predict_full_grid(self, model_name: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        pred_orig = self.scalers['y_scaler'].inverse_transform(
        ...
        if hasattr(model, 'kernel_'):
            sigma_3d_flat[self.grid_data.valid_mask] = sigma_scaled[self.grid_data.valid_mask]
        
        return pred_3d_flat.reshape((nx, ny, nz)), sigma_3d_flat.reshape((nx, ny, nz))
    
    def export_to_grdecl(self, model_name: str, output_dir: Optional[Path] = None) -> Tuple[Path, Path]:
        ...
        def write_grdecl(property_3d: np.ndarray, property_name: str, filepath: Path) -> None:
