...
class UnifiedSPE9Toolkit:
    ...
    def __init__(
        ...
        default_path = Path.home() / "Downloads/Pandey_Ch05_Geomodeling_Code/data/SPE9.GRDECL"
    ...
    def prepare_features(self, *, add_geological_features: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if add_geological_features:
            ...
            feature_names.extend([
                'dist_center', 'depth_factor', 
                'xy_interaction', 'xz_interaction', 'yz_interaction'
            ])
    ...
    def setup_scalers(self, *, scaler_type: str = 'standard') -> Tuple[StandardScaler, StandardScaler]:
    ...
    def train_sklearn_model(self, model: BaseEstimator, model_name: str) -> BaseEstimator:
        """Train scikit-learn model."""
        X_scaled = self.scalers['x_scaler'].transform(self.X_train)
        y_scaled = self.scalers['y_scaler'].transform(self.y_train.reshape(-1, 1)).flatten()
    ...
    def save_model(self, model_name: str, output_dir: Optional[Path] = None) -> None:
        ...
        if self.backend == 'sklearn':
            ...
            # Save GPyTorch model
            torch.save(model['model'].state_dict(), output_dir / f"{model_name}_model.pth")
            torch.save(model['likelihood'].state_dict(), output_dir / f"{model_name}_likelihood.pth")
