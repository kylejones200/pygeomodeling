...
class GRDECLParser:
    ...
    def parse_specgrid(self, content: str) -> Tuple[int, int, int]:
        ...
        specgrid_match = re.search(r'SPECGRID\s*\n\s*(\d+)\s+(\d+)\s+(\d+)', content, re.IGNORECASE)
        if specgrid_match:
        ...
        for prop in properties_to_parse:
            try:
                ...
                if len(prop_data) == total_cells:
                    ...
                    print(f"Warning: {prop} has {len(prop_data)} values, expected {total_cells}")
    ...
    def get_property_slice(self, property_name: str, axis: str = 'z', index: int = 0) -> Optional[np.ndarray]:
...
def load_spe9_data(data_path: str = "/Users/k.jones/Documents/Pandey_Ch05_Geomodeling_Code/data/SPE9.GRDECL"):
...
if __name__ == "__main__":
    ...
    for prop_name, prop_data in data['properties'].items():
        print(f"{prop_name}: min={prop_data.min():.2f}, max={prop_data.max():.2f}, mean={prop_data.mean():.2f}")
